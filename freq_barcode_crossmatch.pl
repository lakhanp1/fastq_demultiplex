...
USAGE: perl freq_barcode_crossmatch.pl <input_file> <used barcode length>
<input_file> should be a TAB delimited file and should have three columns: <sample_id>\t<barcode>\t<fastq_file_path>

fastq_file_path: multiple files can be provided which are , separated
';



if(!defined($ARGV[0]) || !defined($ARGV[1])){
	print STDERR "ERROR: Insufficient arguments\n\n";
	die $usage;
}
...
print join("\t", qw(sample barcodeId barcode usedBarcode totalReads kmerId kmerSeq kmerCount fraction crossmapBarId crossmapBar match kmerMismatches crossmap)),"\n";
...
while(my $line = <$fh>){
	...
	my $usedBar = substr($bc, 0, $ARGV[1]);
	my @fqFiles = split(/,/, $fq);
	my $fopenStr = join(' ', 'gzip -dc', @fqFiles, '|');
	...
	foreach my $ad(sort{$freq{$b} <=> $freq{$a}}keys %freq){
		...
		$matchPattern =~ s/[^0]/*/g;
		print join("\t", $sp, $barcodes{$bc}, $bc, $usedBar, $readCount, "frag$count", $ad, $freq{$ad}, sprintf("%.3f", $freq{$ad}/$readCount), $barcodes{$bc}, $bc, $matchPattern, $adMis, "itself"),"\n";
		...
		foreach my $bar(sort keys %barcodes){
			...
			if($barMis < $adMis && $bar ne $bc){
				...
				$matchPattern =~ s/[^0]/*/g;
				print join("\t", $sp, $barcodes{$bc}, $bc, $usedBar, $readCount, "frag$count", $ad, $freq{$ad}, sprintf("%.3f", $freq{$ad}/$readCount), $barcodes{$bar}, $bar, $matchPattern, $barMis, "different"),"\n";
			}
		}
